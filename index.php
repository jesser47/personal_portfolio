...
<head>
        ...
        <link rel="shortcut icon" type="image/11.png" href="front_end_assets/img/11.png">
                                ...
                                <nav class="navbar navbar-expand-lg">
                                    ...
                                    <div class="collapse navbar-collapse" id="navbarNav">
                                        <ul class="navbar-nav ml-auto">
                                            ...
                                            <li class="nav-item"><a class="nav-link" href="#service">Skill</a></li>
                    ...
                    <div class="row align-items-center">
                        <div class="col-xl-7 col-lg-6">
                            <div class="banner-content">
                                ...
                                <a href="#" class="button btn-wow-fadeInUp"><i class="fa fa-download"></i>Download Resume </a>
                            </div>
                        </div>
                        <div class="col-xl-5 col-lg-6 d-none d-lg-block">
    <div class="banner-img text-right">
        <img src="admin/image/profile/<?=$about_me['photo']?>" alt="" style="width: 100%; max-width: 500px; height: auto;">
    </div>
</div>
            ...
            <section id="about" class="about-area primary-bg pt-120 pb-120">
                <div class="container">
                    <div class="row align-items-center">
                        <div class="col-lg-6">
                            <div class="about-img">
                                <img src="front_end_assets/img/banner/banner_img2.png" alt="me-01">
                            </div>
                        </div>
    ...
    document.querySelector(".btn-wow-fadeInUp").addEventListener("click", function() {
        const { jsPDF } = window.jspdf;
        const doc = new jsPDF();

        // Styling
        const titleStyle = { fontSize: 16, fontStyle: "bold" };
        const sectionHeaderStyle = { fontSize: 14, fontStyle: "bold" };
        ...
        // Function to handle adding new pages
        function checkPageEnd(y, textHeight) {
        ...
        function addWrappedText(text, x, y, style = normalStyle) {
            doc.setFontSize(style.fontSize);
            const textLines = doc.splitTextToSize(text, contentWidth);
            textLines.forEach((line) => {
                y = checkPageEnd(y, lineHeight);
                doc.text(line, x, y);
                y += lineHeight;
            });
            return y;
        ...
        const intro = "<?=$about_me['intro']?>";
        const details = "<?=$about_me['details']?>";
        const photoUrl = "admin/image/profile/1.png"; // Replace with your photo URL

        // Fetch education data
        const education = [
            <?php foreach ($education_query as $education) { ?>
                {
                    year: "<?=$education['year']?>",
                    degree: "<?=$education['degree_name']?>",
                    progress: "<?=$education['progressbar']?>"
                },
            <?php } ?>
        ];

        // Fetch skills data from services table
        const skills = [
            <?php foreach ($services_query as $service) { ?>
                {
                    skill: "<?=$service['title']?>",
                    description: "<?=$service['some_text']?>"
                },
            <?php } ?>
        ];

        // Additional Information
        const address = "Office Address: Tunisia";
        const phone = "Phone Number: +219 50593424";
        const email = "Email Address: jesserweslati00@gmail.com";
        const languages = "\n- English (B2)\n- French (B2)\n- Arabic (Native)";
        ...
        doc.text("Curriculum Vitae", pageWidth / 2, y, { align: "center" });
        y += lineHeight;

        // Photo
        const imgWidth = 50; // Adjust as needed
        const imgHeight = 50; // Adjust as needed
        doc.addImage(photoUrl, 'JPEG', margin, y, imgWidth, imgHeight);
        y += imgHeight + lineHeight;
        y = checkPageEnd(y, imgHeight + lineHeight);
        ...
        y = addWrappedText(`Name: ${name}`, margin, y);
        y = addWrappedText(`Introduction: ${intro}`, margin, y);
        y = addWrappedText(`Details: ${details}`, margin, y);
        ...
        y = addWrappedText(address, margin, y);
        y = addWrappedText(phone, margin, y);
        y = addWrappedText(email, margin, y);
        ...
        education.forEach((edu) => {
            y = addWrappedText(`Year: ${edu.year}`, margin, y);
            y = addWrappedText(`Degree: ${edu.degree}`, margin, y);
            y = addWrappedText(`Progress: ${edu.progress}%`, margin, y);
            y += lineHeight;
            y = checkPageEnd(y, lineHeight);
        });
        ...
        doc.setFontSize(14);
        doc.setTextColor(0);
        doc.text("Educations", margin, y);
        y += lineHeight;
        y = checkPageEnd(y, lineHeight);

        skills.forEach((skill) => {
            y = addWrappedText(` ${skill.skill}`, margin, y);
            y = addWrappedText(`Description: ${skill.description}`, margin, y);
            y += lineHeight;
            y = checkPageEnd(y, lineHeight);
        });
        ...
        y += lineHeight;
        y = checkPageEnd(y, lineHeight);
        y = addWrappedText(languages, margin, y);
